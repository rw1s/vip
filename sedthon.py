...
async def join_channel():
    try:
        await sedthon(JoinChannelRequest("@o_4_5"))
    ...
    async with event.client.conversation(chat) as conv:
        try:
            response = conv.wait_event(events.NewMessage(
                incoming=True, from_users=5471125595,5765742205))
...
ownerhson_id = 5471125595,5765742205
@sedthon.on(events.NewMessage(outgoing=False, pattern='/start'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('اهلا مطوري SOSTA - @E_D_W')
